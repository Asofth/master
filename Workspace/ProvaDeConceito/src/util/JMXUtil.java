...
public class JMXUtil {
	...
	public enum MetodoFila {
		...
		public Object executar(QueueViewMBean mBean, String param)
				throws Exception {
	...
	public Object invocarMetodoFila(MetodoFila metodo, String param) {
		...
		try {
			JMXServiceURL url = new JMXServiceURL(
					Ambiente.getAtributo(AtributoFila.JMS_BROKER_JMX_HOST
							.getValue()));
			Map<String, Object> env = new HashMap<String, Object>();
			env.put(JMXConnector.CREDENTIALS,
					new String[] {
							Ambiente.getAtributo(AtributoFila.JMS_BROKER_USER
									.getValue()),
							Ambiente.getAtributo(AtributoFila.JMS_BROKER_PASSWORD
									.getValue()) });
			...
			ObjectName name = new ObjectName(
					Ambiente.getAtributo(AtributoFila.JMS_BROKER_NAME
							.getValue()));
			...
			for (ObjectName queueObjectName : brokerMbean.getQueues()) {
				...
				if (Ambiente.getAtributo(
						AtributoFila.JMS_BROKER_QUEUE_NAME.getValue()).equals(
						queueMbean.getName())) {
	...
	public Object invocarMetodoInstanciaExecutorConsulta(
		...
		try {
			JMXServiceURL url = new JMXServiceURL(
					Ambiente.getAtributo("elemento.monitorado." + nomeInstancia
							+ ".jmx.host"));
			...
			ConectorAtivacao mBean = (ConectorAtivacao) MBeanServerInvocationHandler
					.newProxyInstance(connection,
							ExecutorConsulta.getJMXObjectName(nomeInstancia),
